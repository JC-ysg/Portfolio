...
float deformation(vec3 pos) {
  return sin(time * 0.05) * 0.6 *
    sin(time * 0.3 + pos.x * 1.9 * sin(pos.y + time * 0.1)) *
    sin(time * 0.2 + pos.y * 1.5) * 
    sin(time * 0.3 + pos.z * 1.5);
}

float scene(vec3 pos)
{
  float t = sphere(pos - vec3(-9.0, 3.0, 10.0), 10.0) + deformation(pos);   
  return t;
}
...
vec3 background() {
  ...
  float gradient = smoothstep(0.6, 0.0, -p.y*0.7);  // Smooth gradient transition 
...
vec3 render(vec3 rayOrigin, vec3 rayDir, vec3 col0)
{
  ...
  float diffuse = max(0.0, dot(-rayDir, norm)*1.15);
...
void main() {
  ...
  vec3 col1 = render(camPos, getCameraRayDir(uv, camPos, camTarget), vec3(0.204,0.204,0.204));
  vec3 col2 = render(camPos1, getCameraRayDir(uv, camPos1, camTarget), vec3(0.102,0.102,0.102));
  vec3 col3 = render(camPos2, getCameraRayDir(uv, camPos2, camTarget), vec3(0.102,0.102,0.102));
  vec3 col4 = render(camPos3, getCameraRayDir(uv, camPos3, camTarget), vec3(0.102,0.102,0.102));
